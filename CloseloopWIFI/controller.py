...
server_address = '192.168.50.101'  # Replace with the IP address of your ESP32
server_port = 10000
...
try:
    ...
    while not done:
        ...
        o_button = joystick.get_button(1)
        x_button = joystick.get_button(0)
        print(int(-left_y*100),int(right_x*100),int((l2_trigger + 1) * 50),int((r2_trigger + 1) * 50),r1_button,l1_button,x_button,o_button)
        # print(b'IO')
        # print(struct.pack('i', int(o_button)))
        ...
        client_socket.send(b'IO')
        client_socket.send(struct.pack('i', int(o_button)))
        
        # # Add a short delay to control the sending rate
        time.sleep(0.15)  # Adjust the delay as needed
